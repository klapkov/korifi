...
func (r *ServiceInstanceRepo) GetServiceInstanceCredentials(ctx context.Context, authInfo authorization.Info, secretGUID string) (map[string]any, error) {
	userClient, err := r.userClientFactory.BuildClient(authInfo)
	if err != nil {
		return map[string]any{}, fmt.Errorf("failed to build user client: %w", err)
	}

	namespace, err := r.namespaceRetriever.NamespaceFor(ctx, secretGUID, ServiceInstanceResourceType)
	if err != nil {
	...
	if err := userClient.Get(ctx, client.ObjectKeyFromObject(credentialsSecret), credentialsSecret); err != nil {
		return map[string]any{}, fmt.Errorf("failed to get credentials secret for service instance: %w", err)
	}
