...
func (h *ServiceInstance) get(r *http.Request) (*routing.Response, error) {
	...
	body := presenter.ForServiceInstance(serviceInstance, h.serverURL)
	body.Included = presenter.IncludedResources(includedResources...)

	return routing.NewResponse(http.StatusOK).WithBody(body), nil
}
...
func (h *ServiceInstance) getCredentials(r *http.Request) (*routing.Response, error) {
	...
	if err != nil {
		return nil, apierrors.LogAndReturn(logger, apierrors.ForbiddenAsNotFound(err), "failed to get service instance credentials")
	}
